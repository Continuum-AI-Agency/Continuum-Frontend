...
- [x] 1.01 Implement LLMNode for text generation (ChatGPT, Gemini, etc.) with output handles for text (note: Completed refactoring of CreativeLibrarySidebar.tsx to use shadcn Sidebar.) (note: Split VideoGenBlock into VeoFastBlock and VideoGenBlock (Veo 3.1). Registered new node type. Verified logic.) (note: Completed LLMNode implementation via StringNode upgrade with inputs and Enrich execution) (note: Relaxed Zod URL validation across 9 files to allow bare domains like 'aws.amazon.com'. Verified with build.) (note: Plan generated. proceeding to summary.) (note: Starting work on Paid Media Refactor (Wave 1).) (note: Refactored Paid Media page with AdAccountSelector, CampaignList, and Tabbed interface. TDD verified.) (note: Starting database migration task) (note: Migration file created manually due to delegation error) (note: Migration created and applied to DB. canvas_rooms table created, canvas_sessions updated with room_id. Data migrated.) (note: Starting implementation of Jaina API proxies.) (note: Implemented Jaina API proxies, updated hook for resilience, and improved UI persistence. CORS errors fixed and analytical state preserved on error.) (note: Verified RLS policies. brand_document_chunks requires auth.uid() to match permissions. Background task currently lacks user context.) (note: Refactored fetchBytes and fetchFromStorage to accept and propagate the authenticated Supabase client. This ensures consistent RLS compliance across DB and Storage operations in the background job. Added path sanitization for storage requests.)
- [x] 1.02 Create VideoNode for video-specific processing and generation (note: Implementing hook) (note: Hook implemented without optimistic updates for V1 stability) (note: Verified schema and data migration. 1 room created for existing brand session.)
...
### Feature 10: Permissions-First Architecture Refactor
Description: Refactor team management to use brand_profiles.permissions as the single source of truth, eliminating synchronization issues with user_onboarding_states.
- [x] 10.01 Add email column to permissions and update RLS for team visibility (note: Starting database migration to add email column and update RLS.) (note: Migration file created: supabase/migrations/20260203140000_refactor_permissions.sql. Added email column, backfill logic, and updated RLS for team visibility.) (note: Refined RLS for brand_profiles and canvas_rooms. Only owners and admins can update brand metadata, while all members can view and access rooms. Fixed potential recursion in policies using security definer functions.)
- [x] 10.02 Refactor brand_invite Edge Function to handle permissions directly and remove sync logic (note: Refactored brand_invite Edge Function. Removed onboarding state sync, added email to permissions upsert, and simplified metadata update. Removed buildDefaultOnboardingState.)
- [x] 10.03 Update getActiveBrandContext and repository to fetch members and invites correctly (note: Updated getActiveBrandContext to include pending invites. Created src/lib/brands/members.ts to fetch members and invites from the permissions and invites tables respectively.) (note: Updated repository and context to support permissions-first fetching. Created members.ts utility for database access.)
- [x] 10.04 Update BrandSettingsPanel UI to display members from the permissions table (note: Updating BrandSettingsPanel and Settings Page to use the new repository methods for members and invites.) (note: Updated BrandSettingsPanel and Settings Page to use permissions-first fetching. Verified fix for ReferenceError and infinite recursion in RLS policies. Final build verification passed.) (note: Updated BrandSettingsPanel UI to disable editing and team management for non-owner/admin roles. Added disabled prop to SettingsLogoUploader. Verified build pass.)
