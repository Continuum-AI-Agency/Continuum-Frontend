...
export const AdNode = memo(({ id, data, selected }: CampaignNodeProps<'ad'>) => {
  ...
  const connectedCreativeAssetTypes = useMemo<CreativeAssetType[]>(() => {
    const creativeNodeIds = edges.filter((edge) => edge.source === id).map((edge) => edge.target);
  ...
  const isFormatCompatibleWithConnectedCreatives = useCallback(
    (adFormat: AdFormat) => {
          ...
          <NodeContent className="space-y-1">
            ...
            <div className="flex items-center gap-1.5">
              <Badge variant="outline" className="text-3xs px-1 py-0 uppercase opacity-70">
                {AD_FORMATS.find((format) => format.value === selectedAdFormat)?.label ||
                  'Single Image'}
              </Badge>
              {data.callToAction && (
                <Badge
                  variant="secondary"
                  className="text-3xs px-1 py-0 uppercase h-4 bg-emerald-500/10 text-emerald-600 border-emerald-500/20"
                >
                  {CALL_TO_ACTIONS.find((c) => c.value === data.callToAction)?.label ||
                    'LEARN MORE'}
                </Badge>
              )}
            </div>
      ...
      <ContextMenuContent className="w-64">
        ...
        <ContextMenuGroup>
          <ContextMenuSub>
            <ContextMenuSubTrigger>
              ...
              <ContextMenuItemInfo
                className="ml-2 mr-4"
                description="Ad format defines how creative is structured in placements."
              />
            </ContextMenuSubTrigger>
          ...
          <ContextMenuSub>
            <ContextMenuSubTrigger>
              ...
              <ContextMenuItemInfo
                className="ml-2 mr-4"
                description="Call to action is the primary next-step prompt shown on the ad."
              />
            </ContextMenuSubTrigger>
            <ContextMenuSubContent className="w-48">
              {CALL_TO_ACTIONS.map((cta) => (
                <ContextMenuCheckboxItem
                  key={cta.value}
                  checked={
                    data.callToAction === cta.value ||
                    (!data.callToAction && cta.value === 'LEARN_MORE')
                  }
        ...
        <ContextMenuGroup>
          <ContextMenuItem onClick={handleDuplicate}>
            ...
            <ContextMenuItemInfo
              className="ml-2"
              description="A duplicate creates another ad with the same current setup."
            />
          </ContextMenuItem>
        </ContextMenuGroup>
        ...
        <ContextMenuItem onClick={handleDelete} className="text-destructive focus:text-destructive">
          ...
          <ContextMenuItemInfo
            className="ml-2"
            description="Delete removes this ad object from the current graph."
          />
        </ContextMenuItem>
      </ContextMenuContent>
    </ContextMenu>
