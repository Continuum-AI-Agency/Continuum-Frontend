  ...
  return {
    ...
    creative: {
      mediaSuggestion: {
        ...
        mediaStatus: "pending",
        assets: [
          { role: "slide_1", order: 1, url: "organic/d1/agent-1.png", bucket: BUCKET, generated: true },
          { role: "slide_2", order: 2, url: "organic/d1/agent-2.png", bucket: BUCKET, generated: true },
        ],
        storyboard: [{ role: "slide_1", storagePath: "organic/d1/preview/a.png", storageUrl: "https://x/a" }],
    ...
    publishingAssets: [
      { role: "slide_1", kind: "image", slideIndex: 1, bucket: BUCKET, storagePath: "organic/d1/agent-1.png", storageUrl: "https://x/agent-1" },
    ],
  };
}
...
describe("buildUserSuppliedContentJson", () => {
  it("stamps user_supplied and mounts BOTH the URL and the re-signable media for a single image", () => {
    ...
    const ms = (next.creative as Record<string, unknown>).mediaSuggestion as Record<string, unknown>;
  ...
  it("builds a user_supplied carousel (slide order = array order) with per-slide publishingAssets", () => {
    const next = buildUserSuppliedContentJson({
      ...
      bucket: BUCKET,
      assets: [
        { role: "slide_1", kind: "image", storagePath: "organic/d1/u1.png", storageUrl: "https://signed/u1" },
        { role: "slide_2", kind: "image", storagePath: "organic/d1/u2.png", storageUrl: "https://signed/u2" },
      ],
    });
  ...
  it("builds a user_supplied reel for an applied video", () => {
    const next = buildUserSuppliedContentJson({
      ...
      bucket: BUCKET,
      assets: [
        { role: "primary", kind: "video", storagePath: "organic/d1/clip.mp4", storageUrl: "https://signed/clip", mimeType: "video/mp4" },
      ],
    });
