...
export default function LoginPage() {
  const { login, sendMagicLink, checkUserStatus, signInWithLinkedIn, signInWithGooglePopup, isPending, isGooglePending, error, clearError, setError } = useAuth();
  ...
  const {
    ...
    resolver: zodResolver(step === "email" ? magicLinkSchema : loginSchema),
  });
  ...
  const onEmailSubmit = async (data: MagicLinkInput) => {
    clearError();
    ...
    if (exists && flow === 'password') {
      ...
      const providers = result.data.providers?.map(p => p.charAt(0).toUpperCase() + p.slice(1)).join(", ");
      setError(`This account uses ${providers}. Please sign in with your social provider.`);
    } else {
            ...
            {!emailSent ? (
              ...
              >
                ...
                <div className="glass-panel rounded-3xl p-8 shadow-2xl border-subtle">
                  ...
                  <form onSubmit={handleSubmit(step === "email" ? onEmailSubmit : onPasswordSubmit)} className="space-y-5 mt-6">
                    {step === "email" ? (
                      <FormInput
                        {...register("email")}
                        id="email"
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-2 p-2 bg-surface rounded-xl border border-subtle">
                          <button
                            ...
                            className="p-1 hover:bg-subtle rounded-lg transition-colors text-secondary"
                          >
                            <ArrowLeft size={16} />
                          </button>
