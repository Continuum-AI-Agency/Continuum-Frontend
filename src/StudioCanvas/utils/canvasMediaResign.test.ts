...
describe('nodeNeedsResign', () => {
  it('returns true when a generated image has a durable pointer but no signed url', () => {
    expect(
      nodeNeedsResign(node({ generatedImageStoragePath: 'p', generatedImageBucket: 'b' })),
    ).toBe(true);
  });
  ...
  it('returns false when the generated image signed url is already present', () => {
    expect(
      nodeNeedsResign(
        node({
          generatedImageStoragePath: 'p',
          generatedImageBucket: 'b',
          generatedImageUrl: 'https://x/p',
        }),
      ),
    ).toBe(false);
  });
  ...
  it('returns true when a generated video has a durable pointer but no signed url', () => {
    expect(
      nodeNeedsResign(node({ generatedVideoStoragePath: 'p', generatedVideoBucket: 'b' })),
    ).toBe(true);
  });
...
describe('resignKey', () => {
  it('derives a stable key from the image durable pointer', () => {
    expect(resignKey(node({ generatedImageStoragePath: 'p', generatedImageBucket: 'b' }))).toBe(
      'img:b\np',
    );
  });
