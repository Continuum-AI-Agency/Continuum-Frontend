...
export async function executeFlow() {
  ...
  for (const node of nanoNodes) {
    ...
    const refImageEdges = edges.filter(
      (e) => e.target === node.id && e.targetHandle === 'ref-images',
    );
  ...
  for (const node of veoNodes) {
    ...
    // Check for images
    const firstFrameEdge = edges.find(
      (e) => e.target === node.id && e.targetHandle === 'first-frame',
    );
    if (firstFrameEdge) {
