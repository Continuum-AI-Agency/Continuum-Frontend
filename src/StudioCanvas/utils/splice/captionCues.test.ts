...
const w = (text: string, startSec: number, endSec: number): CaptionWord => ({ text, startSec, endSec });

describe("rebaseWordsToOutput", () => {
  it("shifts words in a single range to start at zero", () => {
    const out = rebaseWordsToOutput([w("a", 10, 11), w("b", 11, 12)], [{ startSec: 10, endSec: 13 }]);
    expect(out).toEqual([
...
describe("groupWordsIntoCues", () => {
  it("starts a new cue past the word-count cap", () => {
    ...
    const cues = groupWordsIntoCues(words, { maxWordsPerCue: 3, maxCueDurationSec: 100, maxGapSec: 100 });
