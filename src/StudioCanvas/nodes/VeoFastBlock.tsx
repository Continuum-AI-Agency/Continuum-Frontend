...
export function VeoFastBlock({ id, data, selected }: NodeProps<ReactFlowNode<VideoGenNodeData>>) {
  ...
  const promptConnections = flowEdges.filter(
    (edge) => edge.target === id && edge.targetHandle === 'prompt-in',
  ).length;
        ...
        <ContextMenuContent className="w-56">
          ...
          <ContextMenuSub>
            ...
            <ContextMenuSubContent className="w-40">
              <ContextMenuCheckboxItem
                checked={(data.model || 'veo-3.1-fast') === 'veo-3.1'}
                onClick={() => handleModelChange('veo-3.1')}
              >
                Veo 3.1
              </ContextMenuCheckboxItem>
              <ContextMenuCheckboxItem
                checked={(data.model || 'veo-3.1-fast') === 'veo-3.1-fast'}
                onClick={() => handleModelChange('veo-3.1-fast')}
              >
                Veo 3.1 Fast
              </ContextMenuCheckboxItem>
            </ContextMenuSubContent>
