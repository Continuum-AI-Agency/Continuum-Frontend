...
describe("parseMediaFolderKey", () => {
  it("parses a source folder key", () => {
    expect(parseMediaFolderKey(`${MEDIA_SOURCE_FOLDER_PREFIX}canvas`)).toEqual({ source: "canvas" });
  });
...
describe("filterSuggestionsByQuery", () => {
  const suggestions: AgentMentionSuggestion[] = [
    ...
    mediaAssetToMentionSuggestion(makeAsset({ id: "b", title: "Mountain trail", description: null })),
  ];
