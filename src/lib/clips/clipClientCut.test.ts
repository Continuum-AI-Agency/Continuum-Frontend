...
function makeFakeSupabase() {
  const invoke = mock(async (_name: string, { body }: { body: Record<string, unknown> }) => {
    if (body.action === "sign_upload") {
      const path = body.target === "audio" ? "clip-audio/b1/uuid.wav" : `clips/b1/asset-1/${body.sectionIndex}.mp4`
      return { data: { bucket: "media-library", path, token: "tok" }, error: null }
    }
    if (body.action === "register") return { data: { ok: true, status: "ready", assetId: "clip-asset-1" }, error: null }
...
describe("cutAndPersistSection", () => {
  it("cuts → signs → uploads → registers, returns the assetId, and revokes the worker url", async () => {
    ...
    const result = await cutAndPersistSection(
      { brandId: "b1", sourceAssetId: "asset-1", sourceBlob: new Blob([new Uint8Array([0])]), section: SECTION, score: SCORE, onStage: (s) => stages.push(s) },
    ...
    expect(invoke.mock.calls[0][1].body).toMatchObject({ action: "sign_upload", target: "clip", sectionIndex: 1 })
    expect(upload).toHaveBeenCalledTimes(1)
    expect(invoke.mock.calls[1][1].body).toMatchObject({ action: "register", storagePath: "clips/b1/asset-1/1.mp4", durationSec: 26 })
  ...
  it("forwards the selected 720p quality as a 720px short-edge cap to the splice", async () => {
    ...
    await cutAndPersistSection(
      { brandId: "b1", sourceAssetId: "asset-1", sourceBlob: new Blob([new Uint8Array([0])]), section: SECTION, score: SCORE, quality: "720p" },
      { createClient: client, splice: splice as never },
    )
        ...
        functions: {
          invoke: async (_n: string, { body }: { body: Record<string, unknown> }) =>
            body.action === "register"
              ...
              : { data: { bucket: "media-library", path: "clips/b1/asset-1/1.mp4", token: "t" }, error: null },
        },
        storage: { from: () => ({ uploadToSignedUrl: async () => ({ data: {}, error: null }) }) },
      }) as never
    void client
    const splice = mock(async () => ({ blob: new Blob([new Uint8Array([1])]), objectUrl: "blob:x", width: 1, height: 1, durationSec: 5 }))

    await expect(
      cutAndPersistSection(
        { brandId: "b1", sourceAssetId: "asset-1", sourceBlob: new Blob([]), section: SECTION, score: SCORE },
...
describe("extractAndUploadAudio", () => {
  it("extracts the wav, signs an audio upload, uploads, and returns the audio location", async () => {
    const { client, invoke, upload } = makeFakeSupabase()
    const extractAudio = mock(async () => new Blob([new Uint8Array([5, 5])], { type: "audio/wav" }))

    const result = await extractAndUploadAudio(
      { brandId: "b1", sourceAssetId: "asset-1", sourceBlob: new Blob([new Uint8Array([0])]) },
    ...
    expect(upload).toHaveBeenCalledTimes(1)
    expect(result).toEqual({ audioBucket: "media-library", audioStoragePath: "clip-audio/b1/uuid.wav" })
  })
})
