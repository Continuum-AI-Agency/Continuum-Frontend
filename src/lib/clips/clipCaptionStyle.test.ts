...
describe("buildCaptionStyle", () => {
  it("returns the default style for null/empty brand", () => {
    ...
    expect(buildCaptionStyle({ colors: [], typography: { primary: null } })).toEqual(DEFAULT_CAPTION_STYLE);
  });

  it("uses the brand's first valid hex as the highlight, keeps white text + black outline", () => {
    const style = buildCaptionStyle({ colors: ["#1e90ff", "#000000"], typography: { primary: null } });
  ...
  it("passes the brand display font family through (best-effort), even without a usable color", () => {
    ...
    expect(buildCaptionStyle({ colors: ["#1e90ff"], typography: { primary: "  Poppins  " } }).fontFamily).toBe("Poppins");
  });
});
