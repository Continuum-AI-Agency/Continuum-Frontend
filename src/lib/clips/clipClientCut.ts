...
export async function extractAndUploadAudio(
  ...
  const wav = await extract(params.sourceBlob, { signal: params.signal });
...
export async function cutAndPersistSection(
  ...
  try {
    ...
    throw new Error(
      failed.success ? failed.data.message : 'clip-asset register returned an invalid response',
    );
  } finally {
