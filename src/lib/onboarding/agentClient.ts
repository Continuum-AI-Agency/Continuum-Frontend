...
async function assertOk(response: Response): Promise<void> {
  ...
  const base =
    detail && detail.trim().length > 0 ? detail : `${response.status} ${response.statusText}`;
  const message = issues ? `${base} (${issues})` : base;
  throw new Error(message);
}

export async function checkOnboardingAgentHealth(options?: {
  signal?: AbortSignal;
}): Promise<void> {
...
export async function runOnboardingPreview(
  ...
  const handleParsedPayload = (payload: unknown, eventName?: string | null) => {
    ...
    if (typeof parsedPayload === 'string') {
      ...
      try {
        ...
        console.warn('[agentClient] SSE payload was not valid JSON; treating as string.', {
          preview: original.slice(0, 200),
          eventName,
    ...
    const kind =
      ...
      parsedPayload !== null &&
      'kind' in (parsedPayload as Record<string, unknown>)
    ...
    switch (eventName) {
      case 'voice':
        handleDataEvent({ kind: 'data', section: 'voice', data: parsedPayload } as z.infer<
          typeof previewDataEventSchema
        >);
        break;
      case 'audience':
        handleDataEvent({ kind: 'data', section: 'audience', data: parsedPayload } as z.infer<
          typeof previewDataEventSchema
        >);
        break;
      case 'brand_profile':
        handleDataEvent({ kind: 'data', section: 'brand_profile', data: parsedPayload } as z.infer<
          typeof previewDataEventSchema
        >);
        break;
      case 'website':
        handleDataEvent({ kind: 'data', section: 'website', data: parsedPayload } as z.infer<
          typeof previewDataEventSchema
        >);
        break;
      case 'business':
        handleDataEvent({ kind: 'data', section: 'business', data: parsedPayload } as z.infer<
          typeof previewDataEventSchema
        >);
        break;
      case 'status':
