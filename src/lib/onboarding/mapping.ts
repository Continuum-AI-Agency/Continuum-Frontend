...
export function mapOnboardingStateToAgentPayload(
  ...
  const selectedAccounts = Object.entries(state.connections).flatMap(([_, conn]) =>
    (conn.accounts || []).filter((a) => a.selected),
  ) as OnboardingConnectionAccount[];

  const integrationAccountIds = selectedAccounts.map((a) => a.id).filter((id) => UUID_RE.test(id));
