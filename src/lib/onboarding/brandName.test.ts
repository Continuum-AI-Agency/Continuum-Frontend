...
describe("resolveSafeBrandName", () => {
  ...
  it("rejects a login title and falls back to the hostname", () => {
    expect(
      resolveSafeBrandName({ scrapeTitle: "Sign in - Claude", fallbackName: null, url: "https://acme.com/login" })
    ).toBe("acme.com");
  });

  it("rejects a login title but prefers an existing brand name over the hostname", () => {
    expect(
      resolveSafeBrandName({ scrapeTitle: "Sign in - Claude", fallbackName: "Acme", url: "acme.com" })
    ).toBe("Acme");
  });

  it("caps an over-long title", () => {
    const long = "A".repeat(200);
    expect(resolveSafeBrandName({ scrapeTitle: long, fallbackName: null, url: "x.com" }).length).toBe(80);
  });
