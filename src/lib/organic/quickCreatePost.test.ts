...
describe("quickCreatePost client", () => {
  it("posts a validated body with a bearer token and parses the response", async () => {
    ...
    const res = await quickCreatePost(
      ...
      { fetchImpl: fetchImpl as unknown as typeof fetch, getToken: async () => "tok", baseUrl: "http://api" },
    )
  ...
  it("omits the Authorization header when no token is available", async () => {
    ...
    await quickCreatePost(
      ...
      { fetchImpl: fetchImpl as unknown as typeof fetch, getToken: async () => null, baseUrl: "http://api" },
    )
    const init = fetchImpl.mock.calls[0][1] as RequestInit
