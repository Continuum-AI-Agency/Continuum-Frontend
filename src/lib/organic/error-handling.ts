...
export function classifyOrganicError(error: unknown, context: string): ClassifiedError {
  ...
  if (error instanceof TypeError && error.message.toLowerCase().includes('fetch')) {
    return {
      userMessage: 'Connection lost. Check your network.',
      severity: 'error',
      action: 'banner',
      retryable: true,
    };
  ...
  if (lower.includes('meta_login_required') || lower.includes('www.facebook.com')) {
    return {
      userMessage:
        'Your Facebook account requires a security check. Log in to www.facebook.com, complete any verification steps, then reconnect your Instagram integration in Settings.',
      severity: 'error',
      action: 'banner',
      retryable: false,
    };
