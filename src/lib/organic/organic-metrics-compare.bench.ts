...
import { blendMetric, buildSeriesSet, platformBlendKey, type SeriesMode } from './blendAccounts';
...
console.log('\n── 3. Three Instagram accounts: platform blend sums ──');
{
  ...
  assert(both.series.filter((s) => s.kind === 'account').length === 3, 'both: 3 account lines');
  assert(
...
console.log('\n── 4. Mode matrix (one account) never empty ──');
{
  ...
  for (const mode of modes) {
    ...
    console.log(
      `  mode=${mode} → ${series.length} series (${series.map((s) => s.kind).join(',')})`,
    );
  }
}
