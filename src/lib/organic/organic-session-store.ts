...
function isFresh(updatedAt: number | undefined): boolean {
  return typeof updatedAt === 'number' && Date.now() - updatedAt < ORGANIC_SESSION_CACHE_TTL_MS;
}

export const useOrganicSessionStore = create<OrganicSessionStoreState>((set, get) => ({
  sessionsByBrand: {},
  updatedAtByBrand: {},
  ...
  upsertSession: (brandId, session) => {
    ...
    const next =
      idx === -1 ? [session, ...sessions] : sessions.map((s, i) => (i === idx ? session : s));
