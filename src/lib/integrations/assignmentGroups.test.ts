...
function asset(
  partial: Partial<SelectableAsset> & Pick<SelectableAsset, 'asset_pk' | 'type'>,
): SelectableAsset {
  return {
...
function sectionByKey(sections: AssignmentSection[], key: string): AssignmentSection {
  ...
  if (!found)
    throw new Error(`section ${key} not found in [${sections.map((s) => s.key).join(', ')}]`);
  return found;
}
...
describe('filterSection', () => {
  ...
  test('returns all rows and no title match when the query is empty', () => {
    expect(filterSection(section, '  ')).toEqual({
