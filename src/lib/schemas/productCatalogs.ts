...
export const productCatalogRecordSchema = z.object({
  ...
  brandId: z.string().uuid(),
  name: z.string(),
  externalCatalogId: z.string().nullable(),
  businessId: z.string().nullish(),
  catalogStoreId: z.string().nullish(),
  ...
  linkedAdObjectIds: z.array(z.string()).optional().default([]),
  dataFeedEnabled: z.boolean().optional().default(true),
  productTaggingEnabled: z.boolean().optional().default(false),
  syncStatus: productCatalogSyncStatusSchema,
  productCount: z.number().int().min(0).optional().default(0),
  feedCount: z.number().int().min(0).optional().default(0),
  productSetCount: z.number().int().min(0).optional().default(0),
  lastSyncedAt: z.string().datetime({ offset: true }).nullish(),
