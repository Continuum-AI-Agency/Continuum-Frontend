...
describe("isResolvableCreativeRef", () => {
  ...
  it("is true with a creative_id instead of an ad_id", () => {
    const ref = datasetCreativeRefSchema.parse({ creative_id: "cr1", ad_account_id: "act_1", brand_id: "b1" });
    expect(isResolvableCreativeRef(ref)).toBe(true);
  });
  ...
  it("is false without any creative/ad id", () => {
    expect(
      isResolvableCreativeRef(datasetCreativeRefSchema.parse({ ad_account_id: "act_1", brand_id: "b1" })),
    ).toBe(false);
  });
});
