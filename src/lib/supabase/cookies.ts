...
function isSupabaseAuthCookie(name: string) {
  ...
  return SUPABASE_COOKIE_ROOTS.has(root) || root.startsWith('sb-') || root.startsWith('supabase-');
}
...
export async function applySupabaseCookies(
  ...
  const incomingSupabaseNames = new Set(
    deduped.filter(({ name }) => isSupabaseAuthCookie(name)).map(({ name }) => name),
  );
...
async function runDevelopmentSelfCheck() {
  ...
  const handlers = {
    getExisting: () => Array.from(store.entries()).map(([name, { value }]) => ({ name, value })),
