...
describe("Report Parsing Integration", () => {
  it("should parse the user-supplied lead strategist output", () => {
    const payload = {
      ...
      "summary": "Account performance is currently anchored by the 'App AdvantagePlus' Feed campaigns, which are significantly outperforming the 'Selfservice' clusters. The 'Influencer' campaign is the most efficient driver of ROI with a 1.75 ROAS, while the '2024' campaign handles the highest volume of spend effectively. Conversely, the 'Selfservice' campaigns are currently dragging down the overall account ROAS (1.18) due to extremely low conversion efficiency.",
      ...
      "sections": [
        {
          ...
          "content": "The following table ranks the top-performing campaigns by ROAS and conversion volume.",
          "graphs": [
            {
              "type": "bar",
              "title": "ROAS Comparison by Campaign",
              "data": [
                { "label": "Influencer - Feed", "value": 1.75 }
              ]
            }
          ]
        }
  ...
  it("should parse the highly unstructured specialist report payload", () => {
    const payload = {
      ...
      "executive_summary": "Over the last 7 days, your account has maintained a blended ROAS of 1.12...",
      "performance_snapshot": [
        { "label": "Total Spend", "value": 162916.33, "status": "neutral" }
      ],
      "strategy_and_insights": [
        { "title": "Conversion Efficiency Gap", "content": "A critical discrepancy exists..." }
      ],
      "campaign_performance": [
        { "campaign_name": "ANDROID | SELFSERVICE", "spend": 42266.40, "status": "Winner" }
      ],
      "graphs": [
        {
          "type": "bar",
    ...
    if (!result.success) {
      console.error("Specialist Report Parsing Error:", JSON.stringify(result.error.format(), null, 2));
    }
  ...
  it("should parse realtime output_text JSON with main_graph labels/datasets", () => {
    ...
    if (result.success) {
      ...
      expect(data.strategic_recommendations[0].description).toContain(
        "Highest ROAS"
      );
  ...
  it("should parse key_insights, action_plan, performance_table, and keep section summary distinct", () => {
    const payload = {
      summary:
        "The analysis of the campaign reveals a clear performance hierarchy among ad sets.",
      performance_snapshot: [
        {
  ...
  it("should map misspelled reccomendations keys to strategic_recommendations", () => {
    ...
    if (result.success) {
      ...
      expect(data.strategic_recommendations[0].action).toBe(
        "Budget Reallocation"
      );
      expect(data.strategic_recommendations[0].priority).toBe("HIGH");
