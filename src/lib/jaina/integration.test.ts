  ...
  it("processes a full lifecycle of stream events", () => {
    ...
    const events = [
      ...
      { 
        type: "tool.batch", 
        data: { 
          calls: [{ id: "tool_1", name: "fetch_spend", args: {}, metadata: {}, correlation_id: "c1", parent_correlation_id: null }],
          results: [{ id: "tool_1", name: "fetch_spend", ok: true, cached: false, output: { spend: 1000 }, correlation_id: "c1", parent_correlation_id: null }]
        } 
      },
      { type: "handoff.start", data: { correlation_id: "h1", from_scope: "router", to_scope: "analyst", objective: "Analyze", entity_id: null } },
      { 
      ...
      { type: "response.done", data: { id: "resp_1", object: "realtime.response", status: "completed", status_details: null, output: [] } }
    ];
