...
describe('reduceJainaStreamEvent text deltas', () => {
  ...
  it('parses checkpoint_report envelope emitted as raw output_text JSON', () => {
    ...
    const rawCheckpointEnvelope = {
      ...
      report: {
        ...
        performance_snapshot: [{ metric: 'Total Spend', value: '$1,200.00', status: 'neutral' }],
        sections: [
          {
  ...
  it('parses nested content.parts checkpoint JSON emitted as output_text', () => {
    ...
    expect(report.sections[0]?.tables[0]?.rows[0]).toEqual(['Total Spend', '$151,593.91']);
  });
  ...
  it('maps SpecialistReport summary object and section content/table fields', () => {
    ...
    const specialistPayload = {
      ...
      summary: {
        ...
        recommendations: ['Refresh creatives to reduce fatigue.', 'Test broader audiences.'],
      },
      budget: {
    ...
    expect(report.performance_snapshot.some((metric) => metric.metric === 'Total Spend')).toBe(
      true,
    );
...
describe('reduceJainaStreamEvent canonical report events', () => {
  ...
  it('keeps the hydrated report when response.done arrives with failed status', () => {
    ...
    expect(report.executive_summary).toBe('Checkpoint already generated before failure.');
  ...
  it('hydrates objective checklist events and incremental status updates', () => {
    ...
    expect(
      state.objectives.find((objective) => objective.id === 'objective-scope-campaigns')?.status,
    ).toBe('completed');
    ...
    expect(
      state.objectives.find((objective) => objective.id === 'objective-analyze-efficiency')?.status,
    ).toBe('in_progress');
    ...
    expect(
      state.objectives.find((objective) => objective.id === 'objective-finalize')?.status,
    ).toBe('pending');
  });
  ...
  it('hydrates checkpoint summary signal from checkpoint_summary state deltas', () => {
    ...
    state = reduceJainaStreamEvent(state, {
      ...
      data: {
        ...
        delta: {
          latest_checkpoint_summary: 'Shift 15-20% from high-CPC prospecting into lookalikes.',
...
describe('normalizeCheckpointReportPayload strictness', () => {
  ...
  it('keeps object-based summary fields for executive and section summaries', () => {
    ...
    expect(state.status).not.toBe('error');
    const report = asStructuredReport(state);
    expect(report.executive_summary).toBe('Executive summary preserved from summary object.');
    expect(report.sections[0]?.summary).toBe('Section narrative preserved from summary object.');
...
describe('reduceJainaStreamEvent plan capture from heartbeat state.delta', () => {
  ...
  it('builds a minimal plan from a key=value response.plan.delta string', () => {
    ...
    state = reduceJainaStreamEvent(state, {
      type: 'response.plan.delta',
      data: {
        delta: 'plan_id=fallback_abc123; intent=analysis; objectives=2; date_preset=last_7d',
      },
    } as any);
...
describe('hasRenderableStreamContent', () => {
  ...
  it('is true when a pending clarification is present', () => {
    expect(
      hasRenderableStreamContent({
        ...base,
        pendingClarification: { question: 'which campaign?' },
      }),
    ).toBe(true);
  });
});
