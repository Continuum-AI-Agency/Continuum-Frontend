...
const OutOfBoundsDay = ({ day }: OutOfBoundsDayProps) => (
  <div className="relative h-full w-full bg-slate-200/80 p-1 text-slate-600 text-xs">{day}</div>
);
...
export const CalendarBody = ({
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const isSelectedDate = selectedDate ? isSameDay(selectedDate, currentDate) : false;
    ...
    const dayContent = (
      <>
        ...
        {featuresForDay.length > 3 && (
          <span className="block text-slate-700 text-xs">+{featuresForDay.length - 3} more</span>
        )}
      </>
    );
    ...
    if (onSelectDate) {
      days.push(
        <button
          className={cn(
            'relative flex h-full w-full flex-col gap-1 p-1 text-left text-slate-900 text-xs transition-colors cursor-pointer hover:bg-slate-100',
            isSelectedDate && 'bg-sky-200/60 text-slate-900',
          )}
          data-slot="kibo-calendar-day"
          key={day}
    ...
    days.push(
      <div
        className="relative flex h-full w-full flex-col gap-1 p-1 text-slate-900 text-xs"
        key={day}
      >
        {dayContent}
      </div>,
  ...
  return (
    <div className="grid flex-grow grid-cols-7">
      {days.map((day, index) => (
        <div
          className={cn(
            'relative aspect-square overflow-hidden border-t border-r border-slate-500/70',
...
export const CalendarHeader = ({ className }: CalendarHeaderProps) => {
  ...
  return (
    <div className={cn('grid flex-grow grid-cols-7', className)}>
      {daysData.map((day) => (
        <div className="p-3 text-right font-medium text-slate-700 text-xs" key={day}>
          {day}
        </div>
      ))}
    </div>
