...
describe("deriveCanvasConnectionState", () => {
  it("reports workspace-loading while the room list loads and no room is active", () => {
    expect(
      deriveCanvasConnectionState({ ...base, hasRoom: false, roomsLoading: true, status: "INITIALIZING", dbStatus: "INITIALIZING" }),
  ...
  it("reports idle (not connecting) when there is settled-no workspace", () => {
    ...
    expect(
      deriveCanvasConnectionState({ ...base, hasRoom: false, roomsLoading: false, status: "INITIALIZING", dbStatus: "INITIALIZING" }),
    ).toBe("idle");
  });

  it("reports connecting only while a real room is initializing realtime", () => {
    expect(deriveCanvasConnectionState({ ...base, status: "INITIALIZING", dbStatus: "INITIALIZING" })).toBe("connecting");
  ...
  it("alarms with sync-error only when collaborating and the DB channel degrades", () => {
    expect(deriveCanvasConnectionState({ ...base, isCollaborative: true, dbStatus: "TIMED_OUT" })).toBe("sync-error");
  });

  it("reports live-disconnected when collaborating and presence drops", () => {
    expect(deriveCanvasConnectionState({ ...base, isCollaborative: true, status: "ERROR" })).toBe("live-disconnected");
  });

  it("prioritizes a collaborative DB outage over an in-flight save", () => {
    expect(
      deriveCanvasConnectionState({ ...base, isCollaborative: true, dbStatus: "ERROR", isSaving: true }),
    ).toBe("sync-error");
  });
});
