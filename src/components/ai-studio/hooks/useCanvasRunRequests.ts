  ...
  return {
    async claim(runRequestId) {
      const { data, error } = await table()
        ...
        .eq("status", "pending")
        .select("id");
      if (error) throw new Error(`Failed to claim canvas run request ${runRequestId}: ${error.message}`);
      return Array.isArray(data) && data.length > 0;
    },
    async markDone(runRequestId, result) {
      const { error } = await table().update({ status: "done", result }).eq("id", runRequestId);
      if (error) throw new Error(`Failed to finalize canvas run request ${runRequestId}: ${error.message}`);
    },
    async markError(runRequestId, message) {
      const { error } = await table().update({ status: "error", error: message }).eq("id", runRequestId);
  ...
  useEffect(() => {
    ...
    channel
      .on(
        ...
        { event: "INSERT", schema: "brand_profiles", table: "canvas_run_requests", filter: `room_id=eq.${roomId}` },
