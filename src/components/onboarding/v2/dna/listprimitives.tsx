...
export function ChipRow({ label, values, variant }: { label: string; values: string[]; variant: "teal" | "violet" }) {
  return (
    <div>
      <p className="mb-1.5 text-2xs font-semibold uppercase tracking-wide text-muted-foreground">{label}</p>
...
export function BulletList({ label, items }: { label: string; items: string[] }) {
  return (
    <div>
      <p className="mb-1 text-2xs font-semibold uppercase tracking-wide text-muted-foreground">{label}</p>
