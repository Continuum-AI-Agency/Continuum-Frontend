  ...
  return PLATFORM_KEYS.filter((key) => {
    switch (key) {
      ...
      case 'dv360':
        return (
          normalized === 'dv360' ||
          normalized === 'displayvideo360' ||
          normalized === 'display_video_360'
        );
...
function buildAccountsByPlatform(
  selectableAssets: SelectableAsset[],
): Map<PlatformKey, OnboardingConnectionAccount[]> {
...
export function mergeSelectableAssetsConnections({
  ...
  if (selectableAssets.length === 0) {
    ...
    const nextConnections: OnboardingState['connections'] = {
      ...prevConnections,
    } as OnboardingState['connections'];
    ...
    const mergedAccounts = accounts.map((account) => {
      ...
      return existingAccount
        ? {
            ...existingAccount,
            ...account,
            selected: existingAccount.selected ?? (selectedFromState ? true : undefined),
          }
