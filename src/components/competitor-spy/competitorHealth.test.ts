...
describe("toCompetitorHealthInput", () => {
  ...
  it("carries the resolution error through as lastSyncError", () => {
    const input = toCompetitorHealthInput(
      competitor({ metaPageResolutionError: "graph 500" }),
    )
...
describe("competitorHealthChip", () => {
  ...
  it("flags needs_handle when an Instagram handle is missing", () => {
    expect(
      competitorHealthChip(competitor({ organicStatus: "needs_instagram" })).state,
    ).toBe("needs_handle")
  })
  ...
  it("stays in collecting before the first sync", () => {
    expect(competitorHealthChip(competitor({ organicStatus: "ready" })).state).toBe(
      "collecting",
    )
  })
  ...
  it("surfaces a resolution error as sync_error", () => {
    expect(
      competitorHealthChip(competitor({ metaPageResolutionError: "graph 500" })).state,
    ).toBe("sync_error")
  })
})
