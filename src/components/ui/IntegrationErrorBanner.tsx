...
function getConfig(errorCode: IntegrationErrorCode | undefined, platform: string): ErrorConfig {
  const platformLabel = platform === "instagram" ? "Instagram" : "Meta";
  ...
  switch (errorCode) {
    ...
    case "PERMISSIONS_MISSING":
      return {
        title: "Meta ads permissions missing",
        body: (
          <>
    ...
    case "RATE_LIMITED":
      return {
        title: "Meta rate limit active",
        body: "Meta is temporarily rate-limiting requests for this account. No action needed — data will load again automatically in a few minutes.",
      };
      ...
      return {
        ...
        body: (
          <>
            ...
            <strong>business.facebook.com → Security Center</strong> to complete verification.
            Once verified, reconnect your account.
          </>
        ),
    ...
    case "UPSTREAM_UNAVAILABLE":
      return {
        ...
        body: "Meta's API is temporarily unavailable. This usually resolves on its own — no action needed. Try refreshing in a few minutes.",
      };
...
export function IntegrationErrorBanner({ errorCode, platform = "meta", retryAfter }: Props) {
  ...
  return (
    <Callout.Root color="orange" variant="surface" className="my-2">
      <Callout.Text>
