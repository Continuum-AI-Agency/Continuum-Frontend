...
export const GalaxyBackground = memo(function GalaxyBackground({
  ...
  const vignetteTint = isDark
    ? 'var(--bg-galaxy-vignette-dark)'
    : 'var(--bg-galaxy-vignette-light)';
  ...
  return (
    <div className={className ?? ''} aria-hidden>
      ...
      <motion.div
        ...
        animate={
          prefersReduced || !amplitude
            ? undefined
            : { x: [-amplitude, amplitude, -amplitude], y: [0, -amplitude / 3, 0] }
        }
      ...
      <div className="pointer-events-none fixed inset-0 -z-[11] opacity-[0.07] [background-image:radial-gradient(rgba(0,0,0,0.6)_1px,transparent_1px)] [background-size:3px_3px]" />
    </div>
  );
});
