...
// Native-scale gap between the screen top and the dynamic island's bottom edge,
// used to pad live children so the island never covers the post header.
const ISLAND_CLEARANCE = 48;
...
export interface IphoneProps extends HTMLAttributes<HTMLDivElement> {
  src?: string;
  videoSrc?: string;
  // Live, interactive screen content rendered inside the device frame. Takes
  // precedence over src/videoSrc so callers can host real DOM (editable posts).
  children?: ReactNode;
}
...
export function Iphone({ src, videoSrc, children, className, style, ...props }: IphoneProps) {
  ...
  // Punch the screen out of the bezel whenever anything fills it, so the frame
  // reads as a real screen rather than a solid grey panel.
  const hasScreen = hasMedia || hasChildren;
  ...
  return (
    ...
    >
      ...
      {hasChildren && (
        ...
        >
          <div
            ...
            style={{ paddingTop: ISLAND_CLEARANCE }}
          >
            {children}
          </div>
        </div>
      ...
      {/* Decorative device chrome. pointer-events-none lets clicks/edits reach
          the live screen children rendered beneath it. */}
      <svg
