...
export function SamplePreview({ children, overlay, blur = 'md', className }: SamplePreviewProps) {
  return (
    <div className={cn('relative overflow-hidden rounded-lg', className)}>
      <div
        aria-hidden="true"
        inert
        className={cn('pointer-events-none select-none opacity-60', BLUR_CLASS[blur])}
      >
        {children}
      </div>
