...
describe("EmptyState", () => {
  it("renders the outcome-first headline as a heading", () => {
    const { getByRole } = render(
      <EmptyState headline="Connect Instagram to see reach trends" />,
    )
    expect(
      getByRole("heading", { name: /connect instagram to see reach trends/i }),
    ).toBeTruthy()
  })
  ...
  it("renders the description when provided", () => {
    const { getByText } = render(
      <EmptyState
        headline="Get started"
        description="Setup takes about a minute."
      />,
    )
