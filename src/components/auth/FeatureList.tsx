  ...
  return (
    ...
    >
      <div className={`space-y-8 ${containerText}`}>
        <div>
          <h1 className={`text-5xl font-bold leading-tight mb-6 ${headingClass}`}>{title}</h1>
          <p className={`text-xl leading-relaxed ${subtitleClass}`}>{subtitle}</p>
        </div>
          ...
          {features.map((feature, index) => (
            <div key={index} className="flex items-start gap-4">
              <div className="flex-shrink-0 mt-1">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center ${checkBgClass}`}
                >
              ...
              <div>
                ...
                <p className={`${featureDescClass} leading-relaxed`}>{feature.description}</p>
              </div>
            </div>
          ))}
        </div>
