...
/** Everything on a portfolio awaiting a human decision, from the LIST read alone.
 *
 *  Two stores, one number. Recommendations (pause / fatigue / creative) live in
 *  optimizer.recommendations; budget moves live in optimizer.cycle_items. Counting only
 *  the first is what let a portfolio with two actionable moves and zero recommendations
 *  vanish from the Actions queue while the tab badge stayed silent about it.
 *
 *  Deliberately derived from the list row, not from the performance report: the queue's
 *  visibility must not depend on a per-portfolio edge read having succeeded. */
export function pendingActionCount(portfolio: PortfolioListItem): number {
  return portfolio.pending_recommendations + portfolio.pending_budget_moves;
}
