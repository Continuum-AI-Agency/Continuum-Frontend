...
//   1. currency_mismatch — a STRICT MAJORITY of ad sets declare a KPI other than
...
//   2. no_signal — alignment is fine, but not one ad set produced a single
...
//   3. thin_history — signal exists, but the daily series is shorter than the
//      14-day window a confident call needs. Resolves itself with time.
//   4. ready — aligned, tracked, and enough history. A zero-rec cycle here means
//      the allocation is already balanced, not blind.
...
export type SignalVerdict = 'ready' | 'thin_history' | 'currency_mismatch' | 'no_signal';
