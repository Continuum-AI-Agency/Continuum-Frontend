...
import { isJainaChartInput, JainaReportCharts } from './components/JainaReportCharts';
...
export function clampReportHeight(nextHeight: number, maxHeight: number): number {
  ...
  return Math.min(
    Math.max(nextHeight, MIN_REPORT_HEIGHT_PX),
    Math.max(maxHeight, MIN_REPORT_HEIGHT_PX),
  );
}
...
export function JainaReportView({
  ...
  const displayCachedSources = React.useMemo(() => {
    if (!report) return [];
    const combined = [...report.cached_sources, ...(blockDerivedFields?.cached_sources ?? [])];
  ...
  if (status === 'error') {
    return (
      <Alert variant="destructive">
        <AlertDescription>{error ?? 'Unable to render report.'}</AlertDescription>
      </Alert>
    );
  }
  ...
  const reportFrameHeightPx = Math.max(MIN_REPORT_HEIGHT_PX, Math.min(maxAutoHeightPx, 760));
  ...
  return (
    ...
    >
      <ResizablePanelGroup orientation="vertical" className="h-full w-full min-h-0">
        <ResizablePanel defaultSize="84%" minSize={`${Math.round(minReportPanelPercent)}%`}>
          <Artifact className="h-full max-h-full min-w-0 border-white/10 bg-black/20 backdrop-blur-xl shadow-2xl flex flex-col">
            <ArtifactHeader className="bg-white/5 border-b border-white/10 shrink-0">
              ...
              <ArtifactActions>
                <ArtifactAction tooltip="Send to Email" icon={MailIcon} onClick={handleSendEmail} />
                <ArtifactAction
            ...
            <ArtifactContent className="p-0 flex-1 min-h-0 min-w-0 overflow-hidden relative">
              <div className="flex gap-0 items-start relative h-full min-h-0 min-w-0">
                {showSidebarNav ? (
                ...
                <div
                  className={cn(
                    'flex flex-col gap-6 flex-1 min-h-0 min-w-0 overflow-y-auto no-scrollbar h-full',
                  ...
                  <div ref={reportExportRef} className="space-y-6">
                    <div id={sectionId('executive-summary')} className="space-y-4 scroll-mt-20">
                      <div className="flex items-center justify-between">
                        ...
                        <Pill variant="teal" className="uppercase tracking-tighter text-2xs">
                          {report.language || 'EN'}
                        </Pill>
                      </div>
                    ...
                    {displayCachedSources.length > 0 && (
                      <div
                        id="cached-sources"
                        className="pt-6 border-t border-white/5 scroll-mt-20"
                      >
                        <Sources>
