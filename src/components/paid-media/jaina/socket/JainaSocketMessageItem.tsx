...
type JainaSocketMessageItemProps = {
  message: JainaChatMessage;
  activeResponseId: string | null;
  state: JainaStreamState;
  onFeedback?: (feedback: string, planId: string) => void;
};

export function JainaSocketMessageItem({
  message,
  activeResponseId,
  state,
  onFeedback,
}: JainaSocketMessageItemProps) {
                ...
                <Plan status={plan.status} isStreaming={isStreaming}>
                  ...
                  <PlanContent>
                    <div className="space-y-4">
                      {onFeedback && plan.status === "awaiting_approval" && (
                        <PlanFeedback 
                          onFeedback={(f: string) => onFeedback(f, plan.id)} 
                        />
                      )}
                    </div>
                  </PlanContent>
