        ...
        report: {
          ...
          blocks: [
            ...
            },
            {
              ...
              priority: 'secondary',
              notes:
                'The CPA gap between the top campaign and the rest of the account is over 350x.',
...
describe('parsePersistedReportV2Value — full payload coverage (real Supabase fixture)', () => {
          ...
          items: [
            {
              ...
              impact:
                'Expect a significant increase in lead volume while maintaining current spend levels.',
              summary:
                "Transfer 50-100% of the daily budget from 'Todos los idiomas' to 'público inglés 3x1'.",
              cite_ids: [], // stripped
              priority: 'high',
              severity: 'positive',
              item_type: 'action',
              rationale:
                'The English-targeted ad set is 2.5x more efficient at generating leads (messaging connections) for the same spend.',
            },
            {
              title: 'Scale High-Resonance Creative',
              impact:
                'Diversifies lead sources and protects against audience fatigue in the primary ad set.',
              summary:
                "Identify the top 2 videos in the 'público inglés 3x1' set and test them against new audiences.",
              cite_ids: [],
              priority: 'medium',
              severity: 'positive',
              item_type: 'recommendation',
              rationale:
                "The 4.46% CTR suggests the '3x1' video format is highly effective; scaling this creative concept to similar segments is low-risk.",
            },
            {
              title: 'Broad Targeting Inefficiency',
              impact: 'Continuing spend here represents a high opportunity cost for the account.',
              summary:
                "The 'Todos los idiomas' approach is currently failing to reach a relevant audience at a competitive price.",
              cite_ids: [],
              priority: 'high',
              severity: 'risk',
              item_type: 'insight',
              rationale:
                "A CPC of $7.20 is unsustainably high for this account's average, indicating the algorithm is struggling to find buyers in that broad pool.",
            },
          ],
        },
        {
  ...
  it('parses successfully and returns a defined result', () => {
    const parsed = parsePersistedReportV2Value({
      report: undefined,
      content: JSON.stringify(FULL_PAYLOAD),
    });
    expect(parsed).toBeDefined();
  });
  ...
  it('narrative block — preserves body, highlights, block_id, scope, title', () => {
    ...
    const block = (parsed?.blocks ?? []).find(
      (b) => b.block_id === 'efficiency_comparison_narrative',
    );
  ...
  it('data_table block — all 7 columns with key/label/format/align and both data rows', () => {
    ...
    expect(block.columns[4]).toMatchObject({
      key: 'ctr',
      label: 'CTR',
      format: 'percent',
      align: 'left',
    });
    expect(block.columns[6]).toMatchObject({
      key: 'leads',
      label: 'Connections',
      format: 'number',
      align: 'left',
  ...
  it('retains well-formed execution_objectives including the deferred lifecycle status', () => {
    ...
    expect(parsed?.execution_objectives[1]).toMatchObject({
      status: 'deferred',
      title: 'Analyze ad-level performance',
    });
  });
      ...
      sections: [
        {
          heading: 'Pulse',
          scope: 'account',
          summary: 'Performance diverged.',
          highlights: [{ text: 'CPC up 18%.' }],
        },
      ],
    };
