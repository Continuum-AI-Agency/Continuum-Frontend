...
mock.module("./components/JainaMessageItem", () => ({
  JainaMessageItem: ({ message }: { message: Record<string, unknown> }) => {
    ...
    return (
      ...
      >
        <span data-testid={`${String(message.role)}-content`}>
          {String(message.content ?? "")}
        </span>
        <span data-testid={`${String(message.role)}-plan-id`}>
          {plan?.id ?? ""}
        </span>
        <span data-testid={`${String(message.role)}-plan-title`}>
          {plan?.title ?? ""}
        </span>
...
describe("JainaChatSurface integration", () => {
  ...
  it("keeps plan + reasoning visible after response.done snapshot refresh", async () => {
    ...
    await waitFor(() => {
      ...
      const assistantReasoningCount = screen
        .getAllByTestId("assistant-reasoning-count")
        .at(-1);
