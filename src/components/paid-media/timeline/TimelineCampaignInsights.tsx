...
export function TimelineCampaignInsights({ metricsDaily = [], label = 'Campaign trends' }: TimelineCampaignInsightsProps) {
    ...
    return (
        <div className="border-b border-border bg-background/70 px-4 py-3">
            <div className="mb-3 flex flex-wrap items-center gap-4 text-xs">
                ...
                {latest ? (
                    <>
                        <span className="text-muted-foreground">Latest spend: {formatCurrency(latest.spend)}</span>
                ...
                <div className="h-28 rounded-md border border-border bg-background p-2">
                    <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={chartData}>
                            ...
                            />
                            <Line type="monotone" dataKey="spend" stroke="#3b82f6" strokeWidth={2} dot={singlePointDot} />
                        </LineChart>
                    </ResponsiveContainer>
                </div>
                ...
                <div className="h-28 rounded-md border border-border bg-background p-2">
                    <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={chartData}>
                            ...
                            />
                            <Line type="monotone" dataKey="roas" stroke="#10b981" strokeWidth={2} dot={singlePointDot} />
                        </LineChart>
                    </ResponsiveContainer>
                </div>
            </div>
