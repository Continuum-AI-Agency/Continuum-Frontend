...
function resolveAdSetIdFromAction(log: ActionLogMarker): string | null {
  ...
  for (const source of candidates) {
    ...
    const raw = record.meta_adset_id ?? record.metaAdsetId ?? record.adset_id ?? record.adSetId;
...
function resolveCampaignIdFromAction(log: ActionLogMarker): string | null {
  ...
  for (const source of candidates) {
    ...
    const record = source as Record<string, unknown>;
    const raw =
      record.meta_campaign_id ?? record.metaCampaignId ?? record.campaign_id ?? record.campaignId;
    if (typeof raw === 'string' && raw.length > 0) {
      return raw;
    }
  }
...
function getLatestSeriesValue(rows: Array<object>, key: string | undefined): number | null {
  if (!key) return null;
...
function getLatestSeriesPoint(
  ...
  for (let index = rows.length - 1; index >= 0; index -= 1) {
    ...
    const yParsed =
      typeof yValue === 'number' ? yValue : typeof yValue === 'string' ? Number(yValue) : NaN;
...
function buildTimelineFallbackAdSets(
  timelineCampaigns: Array<{
    id: string;
    ad_sets?: Array<{ id?: string; name?: string; status?: string }>;
  }>,
  campaignId: string,
): AdSet[] {
...
function EntityRadarTooltip({
  label,
  comparison,
  isDcoEnabled,
  dcoDeltas,
}: EntityRadarTooltipProps) {
  ...
  return (
    <TooltipContent
      className="w-[320px] border-border/70 bg-background/95 p-3"
      side="top"
      align="start"
    >
      ...
      <div className="mt-2 text-2xs text-muted-foreground">
        Center = no delta, outward = positive, inward = negative.
      </div>
    </TooltipContent>
  );
}
...
function MetricSparkCell({
  ...
  showTarget,
  onClick,
}: MetricSparkCellProps) {
  const currentValue = series.length > 0 ? series[series.length - 1].actual : 0;
  const areaGradientId = React.useMemo(
    () => `spark-area-${metric}-${Math.random().toString(36).slice(2, 8)}`,
    [metric],
  );
  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center justify-between gap-2">
        ...
        <Badge
          variant={ad.effectiveStatus?.toUpperCase() === 'ACTIVE' ? 'default' : 'secondary'}
          className="text-2xs"
        >
...
export function CampaignTimelineWorkspace({
  ...
  const [watchlistGroupMode, setWatchlistGroupMode] = React.useState<'index' | 'objective'>(
    'index',
  );
  ...
  const [focusedAdSet, setFocusedAdSet] = React.useState<
    { campaignId: string; adSetId: string } | undefined
  >();
      ...
      try {
        ...
        while (!cancelled && hasNextPage && page <= 50) {
          ...
          const response = await fetch(
            `${supabaseUrl}/functions/v1/fetch-rule-action-logs?${params.toString()}`,
            {
              headers: {
  ...
  const latestSummaryMetrics = React.useMemo(() => {
    ...
    return {
      ...
      cpa:
        toNumber(summary.total_conversions) > 0
          ? toNumber(summary.total_spend) / toNumber(summary.total_conversions)
          : 0,
  ...
  const watchlistGroups = React.useMemo(() => {
    ...
    const sortedIndexGroups = [...indexGroups].sort((left, right) =>
      left.name.localeCompare(right.name),
    );

    sortedIndexGroups.forEach((indexGroup) => {
      const campaignIdSet = new Set(indexGroup.campaignIds);
      const groupCampaigns = watchlistCampaigns.filter((campaign) =>
        campaignIdSet.has(campaign.id),
      );
      ...
      try {
        ...
        setAdSetState((prev) => ({
          ...
          [campaignId]: {
            ...
            errorMessage: fallbackAdSets.length > 0 ? undefined : message,
          },
        }));
      } finally {
  ...
  const focusedDeltaPct = getEntityDeltaPct(
    selectedMetric,
    focusedComparison,
    focusedIsDcoEnabled,
    latestDcoDeltas,
  ...
  const topChartModel = React.useMemo(() => {
    ...
    if (groupContext?.trends?.length) {
      ...
      const campaignSeries = buildSeriesFromTrends(
        ...
        getEntityDeltaPct(
          selectedMetric,
          focusedCampaign.comparison,
          dcoManagedCampaignIds.includes(focusedCampaign.id),
          latestDcoDeltas,
        ),
        focusedShowTarget,
      );
      ...
      const accountSeries = blocks.map((block) => {
        ...
        return {
          ...
          target: focusedShowTarget
            ? computeTargetValue(metricMap[selectedMetric], focusedDeltaPct)
            : null,
        };
      });
  ...
  const zoomWindowDurationMs = React.useMemo(() => {
    ...
    const first = new Date(
      String((visibleTopChartData[0] as Record<string, unknown>).timestamp ?? ''),
    ).getTime();
    const last = new Date(
      String(
        (visibleTopChartData[visibleTopChartData.length - 1] as Record<string, unknown>)
          .timestamp ?? '',
      ),
    ).getTime();
  ...
  const topPrimaryLineKey = React.useMemo(() => {
    const preferred = topChartModel.lines.find(
      (line) =>
        line.key === 'group_actual' ||
        line.key === 'campaign_actual' ||
        line.key === 'account_actual',
    );
  ...
  const scopedTimelineActionLogs = React.useMemo(() => {
    ...
    return timelineActionLogs.filter((log) => {
      ...
      const campaignId =
        resolveCampaignIdFromAction(log) ?? (scope === 'CAMPAIGN' ? scopeId : null);
  ...
  const adSetSignpostGroupsById = React.useMemo(() => {
    ...
    scopedTimelineActionLogs.forEach((log) => {
      ...
      const adSetId = resolveAdSetIdFromAction(log) ?? (scope === 'ADSET' ? log.scopeId : null);
  ...
  const topTargetValue = React.useMemo(() => topTargetPoint?.y ?? null, [topTargetPoint]);
  ...
  const totalScopedActionCount =
    actionScopeSummary.CAMPAIGN + actionScopeSummary.ADSET + actionScopeSummary.AD;
                ...
                <div className="flex items-center gap-2 text-xs uppercase tracking-wide text-muted-foreground">
                  ...
                  {focusedAdSet ? (
                    <span className="rounded border border-border/70 px-2 py-0.5 normal-case tracking-normal text-foreground">
                      Ad set:{' '}
                      {focusedCampaignAdSets.find((adSet) => adSet.id === focusedAdSet.adSetId)
                        ?.name ?? focusedAdSet.adSetId}
                    </span>
                  ) : null}
                  {focusedAd ? (
                    <span className="rounded border border-border/70 px-2 py-0.5 normal-case tracking-normal text-foreground">
                  ...
                  {groupContext ? (
                    <Badge
                      variant="outline"
                      className="border-blue-400/50 bg-blue-500/10 text-blue-500"
                    >
                      Index
                    </Badge>
                  ) : focusedCampaign ? (
                ...
                <div className="rounded-md border border-border/70 bg-background/60 p-2">
                  <div className="mb-2 flex flex-wrap items-center justify-between gap-2 text-xs">
                    ...
                    <span className="text-muted-foreground">
                      Tooltips include next-bucket KPI shift after each action.
                    </span>
                  </div>
                      ...
                      >
                        ...
                        {topChartModel.lines.map((line) => (
                          <Area
                            ...
                            fill={
                              line.key === topPrimaryLineKey
                                ? `url(#${topAreaGradientId})`
                                : 'transparent'
                            }
                  ...
                  {Array.from({ length: 4 }).map((_, rowIdx) => (
                    ...
                    >
                      ...
                      {Array.from({ length: 6 }).map((_, colIdx) => (
                        <Skeleton
                          key={`campaign-timeline-skeleton-cell-${rowIdx}-${colIdx}`}
                  ...
                  {filteredCampaigns.map((campaign) => {
                    ...
                    const campaignSeriesByMetric = Object.fromEntries(
                      KPI_COLUMNS.map((metric) => {
                        const showTarget =
                          isDcoManaged &&
                          (metric === 'spend' || metric === 'roas' || metric === 'ctr');
                        return [
                          metric,
                          buildSeriesFromTrends(
                            campaign.trends,
                            metric,
                            campaignDeltaByMetric[metric],
                            showTarget,
                          ),
                        ];
                      }),
                      ...
                      <div key={campaign.id} className="p-2.5">
                        ...
                        {isExpanded ? (
                          <div className="mt-2 ml-7 rounded border bg-muted/10 p-2">
                            ...
                            {(adSetLoad.status === 'success' || adSetLoad.status === 'error') &&
                            visibleAdSets.length === 0 ? (
                              <div className="text-xs text-muted-foreground">
                                {activeOnly
                                  ? 'No active ad sets for this campaign.'
                              ...
                              <div className="space-y-2">
                                {[...visibleAdSets]
                                  ...
                                  .map((adSet) => {
                                    ...
                                    const adSetAdsState = adsByAdSet[adSet.id] ?? {
                                      status: 'idle',
                                      ads: [],
                                    };
                                    ...
                                    const adSetSeriesByMetric = Object.fromEntries(
                                      KPI_COLUMNS.map((metric) => {
                                        const deltaPct = getEntityDeltaPct(
                                          metric,
                                          adSet.comparison,
                                          isDcoManaged,
                                          latestDcoDeltas,
                                    ...
                                    const adSetTargetValue = getLatestSeriesValue(
                                      selectedSeries,
                                      'target',
                                    );
                                    const adSetActualValue = getLatestSeriesValue(
                                      selectedSeries,
                                      'actual',
                                    );
                                    const adSetTargetPoint = getLatestSeriesPoint(
                                      selectedSeries,
                                      'timestamp',
                                      'target',
                                    );
                                    const adSetSignposts = (() => {
                                      ...
                                      if (!grouped || selectedSeries.length === 0)
                                        return [] as MarkerPoint[];

                                      const rowByBucket = new Map<
                                        string,
                                        { timestamp: string; y: number }
                                      >();
                                      ...
                                      Array.from(grouped.entries()).forEach(([id, group]) => {
                                        ...
                                        const actionLabel = Array.from(group.actionTypes)
                                          .slice(0, 2)
                                          .join(', ');
                                        const statusLabel = Array.from(group.statuses).join(', ');
                                        ...
                                        const kpiShiftPct = calculateImmediateKpiShiftPct(
                                          rowSeries,
                                          resolution,
                                          group.bucket,
                                        );
                                        ...
                                        {adSetExpanded ? (
                                          <div className="mt-2 rounded border bg-muted/20 p-2">
                                            <div className="mb-2 flex items-center justify-between text-xs">
                                              <span className="text-muted-foreground">
                                                {adSet.name} {labelForMetric(selectedMetric)} trend
                                              </span>
                                              <div className="flex items-center gap-2">
            ...
            <div className="grid gap-2 rounded-md border bg-card/70 p-2 lg:grid-cols-[220px_minmax(0,1fr)]">
              <div className="grid grid-cols-2 gap-1.5">
                {briefingMetrics.map((item) => (
                  <div
                    key={`brief-${item.metric}`}
                    className="rounded border border-border/70 bg-background/70 px-2 py-1.5"
                  >
              ...
              <div className="rounded border border-border/70 bg-background/70 p-2">
                <div className="mb-1 text-xs text-muted-foreground">
                  Aggregated {labelForMetric(selectedMetric)} trend
                </div>
                <div className="h-24">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart
                      data={briefingLineData}
                      margin={{ top: 6, right: 6, left: 0, bottom: 0 }}
                    >
            ...
            <div className="mt-1.5 flex-1 space-y-1.5 overflow-auto pr-0.5">
              {watchlistCampaigns.length === 0 ? (
                ...
                watchlistGroups.map((group) => (
                  <div
                    key={`watchlist-group-${group.id}`}
                    className="rounded border border-border/70 bg-background/70 p-1"
                  >
                    <div className="mb-1 flex items-center justify-between px-1 text-2xs uppercase tracking-wide text-muted-foreground">
                      <span className="truncate">{group.label}</span>
                      <span className={cn(group.isSelectedIndex ? 'text-primary' : undefined)}>
                        {group.count}
                      </span>
                    </div>
              ...
              <div className="max-h-32 space-y-1 overflow-auto">
                {recentActionMessages.length === 0 ? (
                  ...
                  recentActionMessages.map((item) => (
                    <div
                      key={`event-msg-rail-${item.id}`}
                      className="rounded border border-border/60 bg-card px-1.5 py-1"
                    >
