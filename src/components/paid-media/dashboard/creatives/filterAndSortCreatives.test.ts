...
describe('filterAndSortCreatives', () => {
  ...
  it('matches the query case-insensitively over name, creative.title and creative.body', () => {
    ...
    expect(
      filterAndSortCreatives(ads, makeFilters({ query: 'DISCOUNT' })).map((a) => a.id),
    ).toEqual(['title']);
  ...
  it('filters by status using effectiveStatus then status', () => {
    ...
    expect(
      filterAndSortCreatives(ads, makeFilters({ statusFilter: 'paused' })).map((a) => a.id),
    ).toEqual(['paused']);
  });
  ...
  it('sorts metric keys descending and name ascending', () => {
    ...
    expect(
      filterAndSortCreatives(ads, makeFilters({ sortKey: 'clicks' })).map((a) => a.id),
    ).toEqual(['low', 'high']);
