...
export const PlanTitle = ({ children, className, ...props }: PlanTitleProps) => {
  ...
  return (
    ...
    >
      ...
      <span className="flex-1">{isStreaming ? <Shimmer>{children}</Shimmer> : children}</span>
    </CardTitle>
  );
};
...
function StatusIcon({ status }: { status: PlanStatus }) {
  switch (status) {
    ...
    case 'in_progress':
      return (
        <Loader2Icon aria-hidden="true" className="size-4 text-indigo-400 animate-spin shrink-0" />
      );
