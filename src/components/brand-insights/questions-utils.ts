...
function inferPlatformFilter(tokens: string[]): SupportedPlatformKey | undefined {
  if (tokens.some((token) => tokenListIncludes(SUPPORTED_PLATFORMS.youtube.tokens, token)))
    return 'youtube';
  if (tokens.some((token) => tokenListIncludes(SUPPORTED_PLATFORMS.x.tokens, token))) return 'x';
  if (tokens.some((token) => tokenListIncludes(SUPPORTED_PLATFORMS.linkedin.tokens, token)))
    return 'linkedin';
  return undefined;
}
...
function isPlatformToken(token: string) {
  return Object.values(SUPPORTED_PLATFORMS).some((platform) =>
    tokenListIncludes(platform.tokens, token),
  );
}
...
export function getSupportedPlatformKey(
  rawPlatform?: string | null,
): SupportedPlatformKey | undefined {
...
function matchesTextTokens(question: BrandInsightsQuestion, tokens: string[]) {
  ...
  const haystack = [question.question, question.whyRelevant, question.contentTypeSuggestion]
    .filter(Boolean)
    .join(' ')
    .toLowerCase();
...
export function countQuestions(questionsByNiche?: BrandInsightsQuestionsByNiche) {
  return Object.values(questionsByNiche?.questionsByNiche ?? {}).reduce(
    (total, niche) => total + (niche.questions?.length ?? 0),
    0,
  );
}
