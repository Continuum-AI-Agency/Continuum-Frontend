...
describe("normalizePostToolResult — getCompetitorInstagramTopPosts", () => {
  ...
  it("extracts mediaUrl from creative.thumbnailUrl", () => {
    const result = {
      ...
      posts: [{
        ...basePost,
        creative: { thumbnailUrl: "https://cdn.example.com/thumb.jpg", mediaUrl: null, children: [] },
      }],
    };
  ...
  it("falls back to creative.mediaUrl when no children", () => {
    const result = {
      ...
      posts: [{
        ...basePost,
        creative: { thumbnailUrl: null, mediaUrl: "https://cdn.example.com/media.mp4", children: [] },
      }],
    };
    const posts = normalizePostToolResult("getCompetitorInstagramTopPosts", result);
  ...
  it("returns empty array when found=false", () => {
    const posts = normalizePostToolResult("getCompetitorInstagramTopPosts", { found: false, posts: [] });
