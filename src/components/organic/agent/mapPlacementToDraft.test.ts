...
import { mapOneShotPostResponseToCalendarDraft, mapPlacementToDraft } from './mapPlacementToDraft';
...
describe('mapPlacementToDraft', () => {
  it('carries durable publishingAssets through and derives mediaCount from them', () => {
    ...
    expect(draft.backendDraftId).toBe('draft-1');
    expect(draft.clientKey).toBe('draft-1');
  });
  ...
  it('carries the persisted storyboard preview frames through to the draft', () => {
    ...
    expect(draft.mediaSuggestion?.storyboard?.[0].storagePath).toBe('organic/d/preview/1.png');
    expect(draft.mediaSuggestion?.storyboard?.[0].bucket).toBe('brand-profile-assets');
  });

  it('maps a successful one-shot response into an immediately insertable calendar draft', () => {
    const created = mapOneShotPostResponseToCalendarDraft({
      status: 'created',
      draftId: 'one-shot-draft',
      jobId: null,
      caption: 'Your 3-step glass skin routine',
      scheduledAt: '2026-06-01T11:00:00.000Z',
      platform: 'instagram',
    ...
    expect(created?.dayId).toBe('2026-06-01');
    expect(created?.draft.id).toBe('one-shot-draft');
    expect(created?.draft.backendDraftId).toBe('one-shot-draft');
    expect(created?.draft.clientKey).toBe('one-shot-draft');
  });

  it('rejects a one-shot response whose placement cannot be rendered by the calendar', () => {
    const created = mapOneShotPostResponseToCalendarDraft({
      status: 'created',
