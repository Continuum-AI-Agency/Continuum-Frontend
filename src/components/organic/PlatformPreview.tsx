...
export function PlatformPreview({ template, onAssetDrop }: PlatformPreviewProps) {
  ...
  return (
    // biome-ignore lint/a11y/noStaticElementInteractions: creative-asset drag-and-drop target, not a click surface
    <div
    ...
    >
      <div className="flex items-center justify-between">
        <span className="text-sm font-medium">{PLATFORM_LABELS[platform]} Preview</span>
        {showToggle ? (
        ...
        <div className="rounded-[28px] bg-white shadow-inner dark:bg-gray-900">
          <div className="flex items-center gap-3 border-b border-gray-200 px-4 py-3 dark:border-gray-800">
            ...
            <div className="flex-1">
              <span className="block text-sm font-medium">{PLATFORM_LABELS[platform]}</span>
              <span className="block text-xs text-muted-foreground">{template.day_platform}</span>
            </div>
            <Pill variant="muted">{template.format}</Pill>
          </div>
          <div className="relative w-full" style={{ aspectRatio: `${MOBILE_ASPECT_RATIO}` }}>
            {mediaUrl ? (
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 border-b border-gray-200 px-4 py-3 dark:border-gray-800">
        ...
        <div className="flex-1">
          <span className="block font-medium">{template.day_platform}</span>
          <span className="block text-xs text-muted-foreground">{template.format}</span>
        </div>
        <Pill variant="teal">Desktop</Pill>
      </div>
      {mediaUrl ? (
...
function detectPlatform(template: DetailedPostTemplate): OrganicPlatformKey | 'unknown' {
  const haystack = [template.day_platform, template.format, template.type, template.objective]
    .filter(Boolean)
    .join(' ')
    .toLowerCase();
