...
describe("resolveCardMediaKind", () => {
  it("classifies reels, carousels and images", () => {
    ...
    expect(
      resolveCardMediaKind(post({ carouselMedia: [{ id: "a" }, { id: "b" }] }))
    ).toBe("carousel");
...
describe("engagementRate", () => {
  it("is interactions over the larger of reach/views, as a percentage", () => {
    ...
    expect(engagementRate(post({ metrics: { totalInteractions: 100, views: 2000, reach: 0 } }))).toBe(5);
  });
...
describe("getCardMetricSet adaptive primary row", () => {
  ...
  it("reels without hook/watch fall back to Views, Reach, Engagement", () => {
    const reel = post({ mediaProductType: "REELS", metrics: { views: 900, reach: 800, totalInteractions: 40 } });
  ...
  it("carousels lead with Reach, Views, Saves", () => {
    const carousel = post({ mediaType: "CAROUSEL_ALBUM", metrics: { reach: 8100, views: 9400, saved: 320 } });
