...
export function DisabledControl({ hint, children, side, className }: DisabledControlProps) {
  if (!hint) return <>{children}</>;
  return (
    <DisabledReason reason={hint.reason} unlocks={hint.unlocks} side={side} className={className}>
      {children}
    </DisabledReason>
  );
}
