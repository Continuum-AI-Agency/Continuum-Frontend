...
export function DraftHoverCardContent({
  ...
  // The gate is enforced inside usePublishDraft; naming the reason here makes it an
  // affordance rather than a click that silently refuses.
  ...
  return (
    // A preview is a glance, not a page: the card is bounded to most of the viewport and
    // its body scrolls inside that bound. Unbounded, a tall story thumbnail plus a long
    // caption grew the card past the screen and cut its own text off.
    <div className="flex max-h-[70dvh] w-[272px] flex-col overflow-hidden rounded-xl border border-border/80 bg-card shadow-2xl shadow-black/20">
      {/* Media thumbnail */}
      <div className={cn('shrink-0 overflow-hidden', isStory ? 'max-h-[220px]' : 'max-h-[45dvh]')}>
        <DraftCardMedia
          draft={draft}
      ...
      {/* Quick actions */}
      <div className="flex shrink-0 items-center gap-1 border-t border-border/50 px-2.5 py-2">
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation();
        ...
        {canPublish && (
          <button
            ...
            className={cn(
              ...
              'bg-brand-primary/10 text-brand-primary hover:bg-brand-primary/20 disabled:cursor-not-allowed disabled:opacity-50',
            )}
          >
            {isPublishing ? (
