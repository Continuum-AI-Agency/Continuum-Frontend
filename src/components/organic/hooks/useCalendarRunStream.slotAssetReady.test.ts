...
let capturedHandleEvent: ((event: { type: string; data: Record<string, unknown> }) => void) | null = null;

mock.module("@/lib/organic/store", () => ({
  useCalendarStore: (selector: (s: unknown) => unknown) =>
    selector({
      setGridStatus,
      setGridProgress,
      setGridError,
...
mock.module("@/hooks/useRunEventStream", () => ({
  useRunEventStream: (_runId: unknown, handler: (event: { type: string; data: Record<string, unknown> }) => void) => {
