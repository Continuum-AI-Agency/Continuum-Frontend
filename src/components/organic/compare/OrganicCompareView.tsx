        ...
        <>
          {rollupItems.length > 0 ? (
            <div className="flex flex-col gap-1">
              <span className="text-2xs uppercase tracking-wide text-muted-foreground">
                Selection blend (summable metrics)
              </span>
            ...
            <CardHeader className="border-b">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex flex-col gap-1">
                  ...
                  <CardDescription>
                    Decompose accounts, blend platform rollups, or show both. Blend series use a
                    dashed stroke.
                  </CardDescription>
                </div>
            ...
            <CardContent className="pt-4">
              {chartRows.length < 2 || chartSeries.length === 0 ? (
                <p className="py-8 text-center text-sm text-muted-foreground">
                  Not enough daily trend points for{' '}
                  {getOrganicMetric(chartMetric)?.label ?? chartMetric} on the selected accounts
                  {seriesMode !== 'decompose' ? ' (blend needs summable metrics with trends)' : ''}.
                </p>
              ) : (
          ...
          <Card>
            <CardHeader className="border-b">
              ...
              <CardDescription>
                Per-account values and period-over-period deltas. Platform blend rows appear when
                two or more selected accounts share a platform.
              </CardDescription>
            </CardHeader>
                ...
                <TableBody>
                  {visibleAccounts.map((account) => (
                    <TableRow key={keyOf(account)}>
                      ...
                      {activeMetrics.map((metric) => {
                        const available = isMetricAvailableOnPlatform(metric.id, account.platform);
                      ...
                      <TableCell className="pl-4">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-2xs text-muted-foreground">Platform blend</span>
                          </div>
                        </div>
                      </TableCell>
                      {activeMetrics.map((metric) => {
