...
function computePrefetchRange(startDate?: string, endDate?: string): { start: string; end: string } {
...
function mergeTimelineData(
    ...
    const uniqueEvents = Array.from(
        new Map(allEvents.map((event) => [`${event.id || event.date}-${event.adId}-${event.type}`, event])).values()
    );
    uniqueEvents.sort((left, right) => new Date(left.date).getTime() - new Date(right.date).getTime());
...
export function useTimelineBlocks({
    ...
    const fetchResolutionBlocks = useCallback(
        async (targetResolution: TimelineResolution, fetchStart: string, fetchEnd: string): Promise<TimelineBlock[]> => {
