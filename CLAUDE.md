...
- Any persistent client storage that holds brand-scoped state MUST namespace its key with `:b:<brandId>` (use `src/lib/storage/brandScopedStorage.ts`) AND register a teardown handler with `src/lib/storage/storeRegistry.ts` so brand switching wipes stale state.
- Brand integration access is gated by `brand_profiles.brand_integration_grants`. Token reads go through `get_brand_integration_token(brand_id, provider)` (returns decrypted access token via `decrypt_token`); never query `user_integrations` directly from a brand context. Granting MUST go through `grant_integration_to_brand` so the BPIA insert and grant insert remain atomic.
- The `brand_invite` edge function dispatches `create | accept | remove_member | change_role`. Adding a new action requires extending the discriminated `InputSchema` and the action dispatch in `handler()`. `remove_member` cascade-soft-revokes any active grants where the removed user was the `granted_by`; grants by other users survive.
