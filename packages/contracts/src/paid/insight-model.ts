// Pure, deterministic paid-insight model — lifted from the Frontend so it has ONE
// home importable by the Frontend (browser) and the Backend (server generation).
// It ranks each campaign by percentile within the peer set per metric, derives a
// status (strong/watch/risk/unknown) and a severity, and emits templated
// insights. No external deps, so it runs identically in both runtimes.
//
// Input types are minimal structural shapes; the Frontend's richer
// CampaignPerformanceRow / BudgetPacingResponse are structurally assignable to
// them, so existing call sites pass their own types unchanged.

export type CampaignInsightPerfMetric =
  | "spend"
  | "roas"
  | "impressions"
  | "clicks"
...
export type CampaignInsightMetric = CampaignInsightPerfMetric | "pace";
...
export type InsightCampaignInput = {
  id: string;
  name: string;
  metrics?: Partial<Record<CampaignInsightPerfMetric, number>>;
  comparison?: Record<string, { previous?: number; percentageChange?: number } | undefined>;
};

export type InsightBudgetPacingInput = {
  campaigns: Array<{
    campaignId: string;
    campaignName: string;
    pacePct: number;
...
const METRIC_DIRECTIONS: Record<CampaignInsightMetric, CampaignInsightDataPoint["direction"]> = {
  spend: "neutral",
...
const PERFORMANCE_METRICS: CampaignInsightPerfMetric[] = [
  "spend",
  "roas",
  "ctr",
  "cpc",
...
export function buildCampaignInsightDataPoints(params: {
  campaigns: InsightCampaignInput[];
  budgetPacing?: InsightBudgetPacingInput;
  evidenceWindow: string;
  ...
  for (const [campaignId, points] of byCampaign) {
    ...
    if (riskPoints.length > 0) {
      ...
      insights.push({
        ...
        recommendation:
          "Inspect the row, compare against top peers, then validate budget and creative changes before reallocating spend.",
    ...
    if (strongPoints.length >= 2) {
      insights.push({
        ...
        recommendation:
          "Use this campaign as the comparison anchor before scaling or cloning budget strategy.",
