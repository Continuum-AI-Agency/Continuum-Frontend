...
describe('brandStrategySchema', () => {
  ...
  it('requires at least two message pillars', () => {
    expect(() =>
      brandStrategySchema.parse({
        ...FULL_STRATEGY,
        message_pillars: [FULL_STRATEGY.message_pillars[0]],
      }),
    ).toThrow();
  });
  ...
  it('requires at least three personality traits', () => {
    expect(() =>
      brandStrategySchema.parse({
        ...FULL_STRATEGY,
        personality: { ...FULL_STRATEGY.personality, traits: ['one', 'two'] },
      }),
    ).toThrow();
  });

  it('strips unknown top-level keys (mirrors readiness rigor)', () => {
    const parsed = brandStrategySchema.parse({ ...FULL_STRATEGY, sneaky: 'drop me' }) as Record<
      string,
      unknown
    >;
