...
describe("brandDeepSchema (T2 deep lane)", () => {
  ...
  it("round-trips a fully populated deep object", () => {
    const deep = {
      ...
      deep_personas: {
        summary: "ps",
        segments: [{ name: "seg", description: "d", psychographics: [], motivations: [], barriers: [], messaging: [] }],
        content_preferences: [],
        emotional_drivers: [],
        influencer_archetypes: [],
      },
      competitive_frame: {
        top_competitors: [{ name: "C", strategy_summary: "ss", key_messaging: "km", url: "https://c.example" }],
        insights: "i",
        recommendations: "rec",
      },
...
describe("toBrandDeep (strategic analysis -> canonical T2)", () => {
  it("maps the strategic analysis output and drops experimental extras", () => {
    const deep = toBrandDeep(STRATEGIC, { generatedAt: "2026-06-21T00:00:00Z", model: "gemini-3.1-flash-lite" });
