...
describe('brandBookResponseSchema', () => {
  ...
  it('parses a materialized assembled composite of the three sources', () => {
        ...
        documents: [
          {
            id: 'd1',
            name: 'Personas.pdf',
            category: 'audience_persona',
            status: 'ready',
            created_at: '2026-06-22T00:00:00Z',
            excerpt: 'Head of RevOps...',
          },
        ],
    ...
    expect((parsed.assembled?.guidelines[0] as Record<string, unknown>).colors).toEqual({
      primary: '#111',
    });
  ...
  it('carries the effective brand.md, its parsed tokens, and the edited flag', () => {
    const parsed = brandBookResponseSchema.parse({
      ...
      brand_tokens: {
        schema_version: 1,
        brand_name: 'Mocky',
        colors: [{ value: '#111111', role: 'primary' }],
      },
