...
import { type BrandReportResult, brandReportResultSchema } from './brand-report';
...
const FULL_STRATEGY = {
  ...
  promise: {
    ...
    rationale: 'Ops leaders are measured on cadence.',
  },
  ...
  message_pillars: [
    {
      ...
      proof_points: ['deterministic math'],
    },
    {
...
const FULL_GUIDELINES = {
  ...
  messaging_guardrails: {
    ...
    preferred_terms: ['ship'],
  },
  content_pillars: [
    { pillar: 'remote sales rituals', description: 'How distributed teams run cadence.' },
  ],
  formatting: {
    emoji_usage: 'none',
    cta_style: 'Book a 20-min demo',
    hashtag_policy: 'none on LinkedIn',
  },
};
...
function buildResult(overrides?: Partial<BrandReportResult>): BrandReportResult {
  return brandReportResultSchema.parse({
    ...
    structured: {
      ...
      documents: {},
      target_audience: {
        summary: 'Ops + finance leaders who own weekly reporting.',
        segments: [{ name: 'RevOps lead', jtbd: 'Ship the weekly board brief.' }],
      },
      business: null,
...
function sampleTokens(overrides?: Partial<BrandMdTokens>): BrandMdTokens {
  return brandMdTokensSchema.parse({
    ...
    personality: {
      archetype: 'the Sage',
      traits: ['decisive', 'precise', 'plainspoken'],
      descriptors: ['dry'],
    },
    imagery: null,
...
describe('brandMdSaveRequestSchema', () => {
  it('rejects invalid structured front matter but permits prose-only edits', () => {
    expect(
      brandMdSaveRequestSchema.safeParse({ brand_md: '# Prose-only brand story' }).success,
    ).toBe(true);
    expect(
      brandMdSaveRequestSchema.safeParse({
        brand_md: '---\nschema_version: 2\nbrand_name: Acme\n---\n# Story',
      }).success,
    ).toBe(false);
  });
...
describe('parseBrandMd tolerance (never throws)', () => {
  ...
  it("reflects a user-edited hex (the 'fix a wrong scraped color' flow)", () => {
    const doc = serializeBrandMd({ tokens: sampleTokens(), body: 'b' }).replace(
      '#111111',
      '#000000',
    );
...
describe('extractBrandTokens (deterministic, no hallucination)', () => {
  it('pulls palette, typography, voice, personality, audience, logo from the report', () => {
    const tokens = extractBrandTokens(buildResult(), {
      colors: [],
      logoPath: 'brand/mocky/logo.png',
    });
  ...
  it('never invents colors: empty palette and empty kit yields no color tokens', () => {
    ...
    const noPalette = brandReportResultSchema.parse({
      ...
      structured: {
        ...base.structured,
        website: { website_url: base.structured.website.website_url },
      },
    });
  ...
  it('canonicalizes hex colors to lowercase with a leading #', () => {
    ...
    const upper = brandReportResultSchema.parse({
      ...
      structured: {
        ...base.structured,
        website: { ...base.structured.website, palette: { primary: '#ABCDEF' } },
      },
    });
    expect(extractBrandTokens(upper, { colors: [], logoPath: null }).colors[0]?.value).toBe(
      '#abcdef',
    );
  });
});
