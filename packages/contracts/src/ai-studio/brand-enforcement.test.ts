...
import { brandMdTokensSchema, type BrandMdTokens } from "../onboarding/brand-md";
import {
...
function makeTokens(overrides: Record<string, unknown> = {}): BrandMdTokens {
  return brandMdTokensSchema.parse({
    ...
    voice: { tone: "confident", style: "plainspoken", power_verbs: ["build", "ship"], banned_words: ["cheap"] },
...
describe("renderForcedBrandBlock", () => {
  it("filters to only the tagged pieces", () => {
    ...
    expect(block).toContain("Colors (use these exact brand colors): #0a1f44 (primary, Navy), #f5a623 (accent)");
  ...
  it("wraps the block in an authoritative must-comply envelope", () => {
    ...
    expect(block.startsWith("<brand_book>(authoritative brand rules — the generation MUST comply)")).toBe(true);
    expect(block.endsWith("</brand_book>")).toBe(true);
    expect(block).toContain("Voice — Tone: confident. Style: plainspoken. Power verbs: build, ship. Never use: cheap.");
  });
  ...
  it("renders every piece for full and reports wantsLogo when a logo path exists", () => {
    ...
    for (const marker of ["Colors", "Typography", "Voice", "Visual direction", "Personality", "Audience", "Logo"]) {
