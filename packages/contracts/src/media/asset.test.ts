...
describe('mediaSearchRequestSchema', () => {
  it('requires query in text mode', () => {
    expect(mediaSearchRequestSchema.safeParse({ brandId: 'b1', mode: 'text' }).success).toBe(false);
  });
  ...
  it('requires similarToAssetId in similar mode', () => {
    expect(mediaSearchRequestSchema.safeParse({ brandId: 'b1', mode: 'similar' }).success).toBe(
      false,
    );
    expect(
