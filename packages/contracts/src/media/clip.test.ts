      ...
      sections: [
        {
          ...
          words: [
            ...
            { text: 'there', startSec: 0.4, endSec: 0.9 },
          ],
        },
      ],
    });
...
describe('clipGenerateRequestSchema', () => {
  it('requires the source asset and the extracted-audio storage location', () => {
    ...
    expect(
      clipGenerateRequestSchema.safeParse({ brandId: 'b1', sourceAssetId: 'asset-1' }).success,
    ).toBe(false);
  });
});
...
describe('clipGenerationFrameSchema', () => {
  ...
  it('rejects an out-of-enum stage', () => {
    expect(clipGenerationFrameSchema.safeParse({ type: 'stage', stage: 'nope' }).success).toBe(
      false,
    );
  });
});
...
describe('temporary caption audio cleanup contract', () => {
  it('round-trips the scoped request and deleted response', () => {
    expect(
      clipDeleteAudioRequestSchema.safeParse({
        brandId: 'b1',
        audioBucket: 'media-library',
        audioStoragePath: 'clip-audio/b1/temp.wav',
      }).success,
    ).toBe(true);
    expect(clipDeleteAudioResponseSchema.safeParse({ ok: true, status: 'deleted' }).success).toBe(
      true,
    );
  });
});
