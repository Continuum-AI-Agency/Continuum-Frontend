...
export function scoreAdSet(
  ...
  // --- Composite (calibrated window weights, renormalized) ---------------
  // Default: binary significance gate (Excel). P1 precision weighting replaces the
  // gate with a continuous reliability per window — calibrated weight × precision,
  // precision = events/(events+precisionK) — so low-event windows are smoothly
  // down-weighted, not discarded. The calibrated weights (the essence) are kept.
  let w3eff: number, w7eff: number, w14eff: number;
  if (cfg.toggles.precisionWeighting) {
    const k = cfg.precisionK > 0 ? cfg.precisionK : 10;
    const prec = (events: number, score: number) => (score > 0 ? events / (events + k) : 0);
    w3eff = weights.d3 * prec(kpiEvents(s.windows.d3, cfg), score3d);
    w7eff = weights.d7 * prec(kpiEvents(s.windows.d7, cfg), score7d);
    w14eff = weights.d14 * prec(kpiEvents(s.windows.d14, cfg), score14d);
  } else {
    w3eff = score3d > 0 && windowCounts(s.windows.d3, cfg) ? weights.d3 : 0;
    w7eff = score7d > 0 && windowCounts(s.windows.d7, cfg) ? weights.d7 : 0;
    w14eff = score14d > 0 && windowCounts(s.windows.d14, cfg) ? weights.d14 : 0;
  }

  const num = w3eff * score3d + w7eff * score7d + w14eff * score14d;
  const den = w3eff + w7eff + w14eff;
