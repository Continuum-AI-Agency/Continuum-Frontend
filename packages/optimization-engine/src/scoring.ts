 ...
 * Tier 1 (1/CPP) when purchases >= minPurchasesSignif; otherwise the Tier-2
 * ATC-rate proxy. Returns 0 when the window has no usable data.
 *
...
export function windowCounts(m: WindowMetrics, cfg: EngineConfig): boolean {
  if (cfg.toggles.significanceGate) {
    const events = m.purchases > 0 ? m.purchases : m.addToCarts;
...
function priorBlock(d3: WindowMetrics, d7: WindowMetrics): WindowMetrics {
  return {
    spend: Math.max(0, d7.spend - d3.spend),
    purchases: Math.max(0, d7.purchases - d3.purchases),
    addToCarts: Math.max(0, d7.addToCarts - d3.addToCarts),
    clicks: Math.max(0, d7.clicks - d3.clicks),
    impressions: Math.max(0, d7.impressions - d3.impressions),
  };
}
...
export function scoreAdSet(s: AdSetSnapshot, cfg: EngineConfig): ScoreBreakdown {
