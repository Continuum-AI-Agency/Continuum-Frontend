...
import { buildBrandInsightsProgressSteps, BRAND_INSIGHTS_PROGRESS_STAGE_ORDER } from "../../src/lib/brand-insights/progress.ts";
...
test("buildBrandInsightsProgressSteps supports awaiting_strategic_analysis stage", () => {
  const steps = buildBrandInsightsProgressSteps({ stage: "awaiting_strategic_analysis", status: "pending" });
  const statuses = Object.fromEntries(steps.map((step) => [step.id, step.status]));
