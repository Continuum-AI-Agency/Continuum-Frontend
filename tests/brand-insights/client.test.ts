...
test('generateBrandInsights sends required window fields when caller omits them', async () => {
  ...
  assert.equal(
    String(capturedBody.window_start).slice(0, 10),
    String(capturedBody.week_start_date),
  );
...
test('subscribeToBrandInsightsJob falls back to polling when stream origin differs', async () => {
  const originalFetch = globalThis.fetch;
  const originalEventSource = (globalThis as typeof globalThis & { EventSource?: unknown })
    .EventSource;
  ...
  (globalThis as typeof globalThis & { EventSource: unknown }).EventSource =
