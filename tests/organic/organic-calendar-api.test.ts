...
import type { CalendarGenerationRequest, CalendarGenerationEvent } from "@/lib/organic/calendar-generation";
  ...
  describe("streamCalendarGeneration", () => {
    ...
    test("parses error events from NDJSON stream", async () => {
      ...
      const errorData = encoder.encode(
        JSON.stringify({ type: "error", code: "GENERATION_FAILED", message: "Failed to generate" }) + "\n"
      );
    ...
    test("throws error on non-ok response", async () => {
      ...
      await expect(
        streamCalendarGeneration(payload, () => {})
    ...
    test("throws error when response has no body", async () => {
      ...
      await expect(
        streamCalendarGeneration(payload, () => {})
      ).rejects.toThrow("Failed to start calendar generation");
    });
