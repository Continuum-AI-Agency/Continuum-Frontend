...
function createEmptyBrandSummary(): BrandIntegrationSummary {
  return PLATFORM_KEYS.reduce(
    ...
    {} as Record<PlatformKey, { accounts: BrandIntegrationSummary[PlatformKey]['accounts'] }>,
...
describe('selectable assets schemas', () => {
  ...
  test('mergeSelectableAssetsWithBrandSummary includes assigned brand accounts missing from selectable assets', () => {
    ...
    expect(
      flat.some((asset) => asset.integration_account_id === 'bbbb2222-2222-4222-8222-222222222222'),
    ).toBe(true);
    expect(
  ...
  test('filterSelectableAssetsByAccountIds removes accounts outside brand grants', () => {
    ...
    expect(
      filtered.providers.meta?.hierarchy?.integrations?.[0]?.businesses?.[0]?.ad_accounts?.[0]
        ?.ad_account,
    ).toBeNull();
  });
  ...
  test('getMetaSelectableAdAccountBundles groups by ad account and ignores businesses', () => {
    ...
    expect(bundles?.ad_accounts[0]?.assets.map((a) => a.name)).toEqual([
      'IG 1',
      'Page 1',
      'Page 2',
      'Threads 1',
