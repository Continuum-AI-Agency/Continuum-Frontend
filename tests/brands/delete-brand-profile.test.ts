...
test('invokeDeleteBrandProfile throws when function returns an error', async () => {
  ...
  await assert.rejects(() => invokeDeleteBrandProfile('brand-1', client), /Edge failure/);
});
...
test('invokeDeleteBrandProfile throws when response body contains an error', async () => {
  ...
  await assert.rejects(() => invokeDeleteBrandProfile('brand-1', client), /Forbidden/);
});

test('invokeDeleteBrandProfile forwards bearer token to the edge function', async () => {
  let receivedOptions: { body: { brandId: string }; headers?: Record<string, string> } | null =
    null;
  const client = createClient({
    auth: {
      getSession: async () => ({
