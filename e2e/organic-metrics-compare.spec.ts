...
const OWNER_EMAIL =
  process.env.ORGANIC_METRICS_BENCH_EMAIL?.trim() || "duane@continuumai.agency";
const BRAND_ID =
  process.env.ORGANIC_METRICS_BENCH_BRAND_ID?.trim() ||
  "46f0deba-013f-4bd9-a70e-2526677a831d"; // La Chica de la IA
const OWNER_USER_ID =
  process.env.ORGANIC_METRICS_BENCH_USER_ID?.trim() ||
  "bc29b6ab-8711-4a5e-9deb-a3182844b16c";
...
async function pinActiveBrand() {
  ...
  if (!url || !key) {
    console.log(
      "[organic-metrics-compare] skip pinActiveBrand (missing service role / URL)",
    );
    return;
  }
  const res = await fetch(
    `${url}/rest/v1/user_brand_preferences?on_conflict=user_id`,
    {
      method: "POST",
      headers: {
        apikey: key,
  ...
  console.log(
    `[organic-metrics-compare] pinned active brand ${BRAND_ID} for ${OWNER_EMAIL}`,
  );
}
...
test.describe("organic metrics compare (headless)", () => {
  test("La Chica de la IA — selector + Decompose/Blend/Both", async ({ browser }) => {
    ...
    try {
      ...
      await expect(page.locator(tid("series-mode-blend"))).toHaveAttribute(
        "data-state",
        "active",
      );
      ...
      await expect(page.locator(tid("series-mode-both"))).toHaveAttribute(
        "data-state",
        "active",
      );
      ...
      if ((await igChip.count()) > 0) {
        ...
        await expect(
          page.locator(tid("organic-metrics-dashboard")),
        ).not.toContainText("Select an account above", { timeout: 90_000 });
